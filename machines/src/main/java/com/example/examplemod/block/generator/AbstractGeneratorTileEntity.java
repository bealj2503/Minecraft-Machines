...
public abstract class AbstractGeneratorTileEntity extends AbstractMachineBaseTileEntity {
    ...
    public void tick() {
        if (canRun()) {
            ...
            if(doesSeeSky() && calculateSunIntensity() > 0F){
                sendUpdate(getActiveState(), true);
        ...
        if (burnTime > 0) {
            --burnTime;
            energy.createEnergy(getEnergyCreatedPerTick());
        } else if(!doesSeeSky() || calculateSunIntensity() > 0F){
            sendUpdate(getInactiveState(), false);
        }

        super.tick();
    }
    protected abstract float calculateSunIntensity();
    protected abstract boolean doesSeeSky();
