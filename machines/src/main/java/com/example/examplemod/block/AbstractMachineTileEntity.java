...
public abstract class AbstractMachineTileEntity<R extends IRecipe<?>> extends AbstractMachineBaseTileEntity implements IMachineInventory {
    ...
    protected AbstractMachineTileEntity(TileEntityType<?> typeIn, int inventorySize) {
        super(typeIn, inventorySize, 100000, 500, 0);
    }

    @Override
    public EnergyStorageMy getEnergyMy() {
        return super.getEnergyMy();
    }
    ...
    public void tick() {
        ...
        if (recipe != null && canMachineRun(recipe)) {
            System.out.println("Apparently the machine cant run?");
            // Process
            ...
            if (recipe == null) {
                //System.out.println("is this the reason why?");
                progress = 0;
            }
            setInactiveState();
        }
